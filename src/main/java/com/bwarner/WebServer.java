...
public class WebServer extends Thread {
	...
	/** start server with valid arguments for port number and threads **/
	public void startServer(int portNumber, int threads) throws IOException {
		ServerSocket socket = new ServerSocket(portNumber);
		System.out.println("Web server working on port " + portNumber + " with " + threads +" threads.");
		ExecutorService executor = Executors.newFixedThreadPool(threads);
		while (true) {
			//executor.submit(new HandleRequests(socket.accept()));
		}
	}
